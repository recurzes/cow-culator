"""Extract frames from videos in a directory (FFmpeg, ~2 fps)."""
...
def get_ffmpeg_path() -> str | None:
    if shutil.which("ffmpeg"):
        return "ffmpeg"
    local_ffmpeg = Path(__file__).resolve().parent.parent.parent / "ffmpeg_bin" / "ffmpeg.exe"
...
def extract_frames(directory_path: str | Path) -> None:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Extract 2 frames per second from all videos in a target directory."
    )
